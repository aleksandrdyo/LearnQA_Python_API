...
class TestUserDelete(BaseCase):
    ...
    def test_delete_other_user(self):
        ...
        # CHANGE NEGATIVE
#Добрый вечер. Это задание с небольшим подвохом. При формировании теста -
# всегда надо убеждаться, что сущности, которые создаются/удаляются/изменяются
# действительно подвергаются изменениям. Сервер может радостно ответить 200,
# но на самом деле не сделать ничего или сделать не то, что ожидается.
# Для этого необходимо в конце тестов делать доп. проверку на факт
# произведенных изменений в данном случае запросить данные пользователя или попытаться им авторизоваться.
